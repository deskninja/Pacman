...
public class Graph {
	...
	/**
	 * made for each char in the maze tracks and stores information for the Graph
	 * class to use
	 *
	 * @author Jonathan Oliveros and Joshua Wells
	 */
	private class Node {
		...
		int dist = INFINITY;
		/**
		 * number of times this Node is checked to see if it is closer than the current
		 * path
		 */
		int timesVisited;
		/**
		...
		int indexInMatrix;
		/**
		 * position of this Node in {@code maze} format is row space column example: "1
		 * 2"
		 */
		String position;
	...
	private char[][] maze;
	/**
	 * maze from the text file with "." in place of " " to represent a shortest path
	 * solution to the maze
	 */
	private char[][] newMaze;
	/**
	 * stores connections between nodes as 1 iff connected or {@code INFINITY} iff
	 * not connected
	 */
	private int[][] matrix;
	/**
	 ...
	 * @requires X represents a wall, a space represents an open space, S represents
	 *           the start, and G represents the end.
	 * @requires the edges of the maze are all X and the maze contains one S and one
	 *           G
	 */
	public Graph(String file) {
		...
		for (int i = 0; i < (this.rows) * (this.cols); i++) {
			if (maze[position[0]][position[1]] == 'G' || maze[position[0]][position[1]] == 'S'
	...
	/**
	 * determines if there should be an edge between two nodes addEdge iff the space
	 * is not an X and is a valid position
	 * 
	 * @param position int array position of the node in the maze
	...
	/**
	 * determines if position is an open space in the maze (start and end are
	 * considered open spaces) and if position is in the maze
	 * 
	 * @param position int array position of the node in the maze
